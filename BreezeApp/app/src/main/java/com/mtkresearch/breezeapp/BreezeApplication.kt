...
import com.mtkresearch.breezeapp.domain.usecase.breezeapp.ConnectionUseCase
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                connectionUseCase.initialize().collect { state ->
                    when (state) {
                        is com.mtkresearch.breezeapp.domain.model.breezeapp.ConnectionState.Connected -> {
                            android.util.Log.i("BreezeApplication", "BreezeApp Engine connected successfully")
                        }
                        is com.mtkresearch.breezeapp.domain.model.breezeapp.ConnectionState.Failed -> {
